...
using library branch `opencsd-0v002` (decode library only) and the latest perf
branch `perf-opencsd-4.7-rc1` (decode library + perf tools) on the
...
`perf-opencsd-4.7-rc1` needs to be downloaded to the target system where
traces are to be collected.  This branch is an upstream v4.7-rc1 kernel
...
get all the components branch `perf-opencsd-4.7-rc1` needs to be
obtained:
